...
<div class="card" style="max-width:480px;margin:4rem auto;">
	<h2>🔐 管理面板验证</h2>
	<p style="color:#94a3b8;font-size:0.875rem;margin-bottom:1.5rem;">请输入 Cloudflare API Token 以访问管理面板。</p>
	<label>Cloudflare API Token</label>
	<input type="password" id="admin-token-input" placeholder="粘贴你的 API Token" autocomplete="off" />
	<button class="btn-primary" onclick="verifyToken()" style="width:100%;">验证</button>
...
async function verifyToken() {
	...
	if (!token) { result.className = "result error"; result.textContent = "请输入 Token"; return; }
	result.className = "result info";
	result.textContent = "验证中...";
	try {
		const resp = await fetch("/admin/verify", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({ token })
		});
		if (resp.ok) {
			sessionStorage.setItem("ariya_admin_token", token);
			location.reload();
		} else {
			const data = await resp.json();
			result.className = "result error";
			result.textContent = data.error || "验证失败";
		}
	} catch (e) {
		result.className = "result error";
...
document.getElementById("admin-token-input").addEventListener("keydown", e => { if (e.key === "Enter") verifyToken(); });
</script>
`;
...
<div class="card">
	...
	<div id="key-usage" style="display:none;margin-top:1rem;border-top:1px solid #334155;padding-top:1rem;">
		...
		<div style="margin-bottom:1rem;">
			...
			<div style="background:#0f172a;padding:0.75rem;border-radius:0.375rem;font-size:0.8125rem;font-family:monospace;margin-bottom:0.25rem;">
...
export function isAdminAuthed(env: Env, token: string): boolean {
	if (!env.ADMIN_KEY) return false;
	if (token.length !== env.ADMIN_KEY.length) return false;
	let result = 0;
	for (let i = 0; i < token.length; i++) {
		result |= token.charCodeAt(i) ^ env.ADMIN_KEY.charCodeAt(i);
	}
	return result === 0;
}

export async function handleVerify(env: Env, body: { token?: string }): Promise<Response> {
	if (!body.token) {
		return new Response(JSON.stringify({ error: "Missing token" }), { status: 400 });
	}
	if (isAdminAuthed(env, body.token)) {
		return new Response(JSON.stringify({ success: true }));
	}
	return new Response(JSON.stringify({ error: "Invalid token" }), { status: 403 });
}

export function renderAdminPage(env: Env, providedToken: string): Response {
	const isAuthed = isAdminAuthed(env, providedToken);
	const content = env.ADMIN_KEY
		? (isAuthed ? DASHBOARD_PAGE : LOGIN_PAGE)
		: `<div class="card" style="max-width:480px;margin:4rem auto;"><h2>⛔ 管理面板未启用</h2><p style="color:#94a3b8;font-size:0.875rem;margin-top:0.5rem;">请设置 <code style="background:#0f172a;padding:0.125rem 0.375rem;border-radius:0.25rem;">ADMIN_KEY</code> 环境变量后重新部署。<br/><br/>本地开发：在 <code style="background:#0f172a;padding:0.125rem 0.375rem;border-radius:0.25rem;">.dev.vars</code> 中添加 <code style="background:#0f172a;padding:0.125rem 0.375rem;border-radius:0.25rem;">ADMIN_KEY=你的Cloudflare APIToken</code><br/>生产部署：<code style="background:#0f172a;padding:0.125rem 0.375rem;border-radius:0.25rem;">npx wrangler secret put ADMIN_KEY</code></p></div>`;
...
<script>
const ADMIN_TOKEN = "${isAuthed ? providedToken : ""}";
...
async function loadLogs() {
	...
	try {
		const token = ADMIN_TOKEN || sessionStorage.getItem("ariya_admin_token") || "";
		const resp = await fetch("/admin/logs?token=" + encodeURIComponent(token));
		const data = await resp.json();
		if (resp.status === 401) {
			container.innerHTML = '<div class="result error">未授权，请刷新页面重新验证</div>';
			return;
		}
