...
- **📋 最近提交** — 在控制台查看最近 50 条日志（含错误次数）

### 日志浏览

侧边栏点击 **日志浏览** 进入。页面展示所有日志的全部字段（时间、Mod、版本、游戏版本、异常消息、堆栈、游戏状态、操作系统等），支持：
...
- **基本信息** — 时间、Mod、版本、游戏版本、错误次数、操作系统、Hash
- **异常信息** — 完整堆栈跟踪，首行红色高亮
- **游戏状态** — 自动解析 JSON 并以可读表格展示（场景、种子、进阶等级、楼层、角色、血量等）

> 两种认证方式可同时启用。Access 优先。
